-module(vodkatv_eqc_statem_pilot).
...
purchase_television_product_pre(S) ->
    false. % YOU NEED TO WRITE THIS, but note that we are modelling behaviour from a single user point of view, hence only one user can use a television at a time. 
...
add_tv_channel_to_favourite_channels_args(S) -> 
    [ok]. % YOU NEED TO WRITE THIS, probably using ?SUCHTHAT(X,Generator,Condition)
