-module(vodkatv_eqc_fsm_pilot_alex2).
...
-record(state, {
    ...
    change_password_code, % code to change password
    ...
    purchase_television, % id of the television purchase, used for cancellation
...
password_recovery(UserId) ->
    ...
    {ok, R} = vodkatv_connector:get_password_recovery_code(UserId),
    R.
...
password_recovery_next(_From, _To, S, V, [UserId]) ->
    S#state{change_password_code = {UserId,V}}.

password_recovery_post(_From, _To, _S, [_UserId], {error, _}) ->
    false;
password_recovery_post(_From, _To, _S, [_UserId], _R) ->
    true.
...
change_password_from_code_pre(_From, _To, S, _Args) ->
    S#state.change_password_code /= undefined.

change_password_from_code_args(_From, _To, S) ->
    case S#state.change_password_code of
	{_UserId,RecoveryCode}->[RecoveryCode, gen_password()];
	_ -> undefined
    end.

change_password_from_code_next(_From, _To, S, _V,[PasswordRecoveryCode, Password]) ->
    {UserId,_RecoveryCode}=S#state.change_password_code,
    S#state{change_password_code = undefined,valid_users=[{UserId,Password}|proplists:delete(UserId,S#state.valid_users)]}.
...
purchase_television_product_next(_From, _To, S, V, [_Token, ProductId]) ->
    S#state {
...
add_tv_channel_to_favourite_channels_args(_From, _To, S) ->
    [S#state.current_token, eqc_gen:oneof(S#state.tv_channels)].
